...
class Digimon{
    ...
    private:
        ...
        unsigned long hungerTimerMs = 0; // hunger decay timer
        unsigned long healthTimerMs = 0; // periodic hp update
        unsigned long sicknessRollTimerMs = 0; // periodic sickness roll
        unsigned long actionCooldownFeedMs = 0;
        unsigned long actionCooldownCleanMs = 0;
        unsigned long actionCooldownTrainMs = 0;
    ...
    public:
        ...
        void setHp(uint8_t _hp){hp=_hp;};
        void setSick(boolean _s){isSick=_s;};
        void cleanPoop(){ numberOfPoops = 0; };
        void cure(){ isSick=false; sicknessCounter=0; };
        ...
        uint8_t getHp(){return hp;};
        boolean getIsSick(){return isSick;};
        ...
        void addHp(int8_t d){ if(d<0 && hp < (uint8_t)(-d)) hp=0; else { int v = (int)hp + d; if(v<0) v=0; if(v>100) v=100; hp=(uint8_t)v; } }

        // cooldown helpers
        bool canFeed() const { return actionCooldownFeedMs==0; }
        bool canClean() const { return actionCooldownCleanMs==0; }
        bool canTrain() const { return actionCooldownTrainMs==0; }
        void tickCooldowns(unsigned long delta){
            auto dec=[&](unsigned long &t){ if(t>delta) t-=delta; else t=0; };
            dec(actionCooldownFeedMs); dec(actionCooldownCleanMs); dec(actionCooldownTrainMs);
        }
        void setFeedCooldown(unsigned long ms){ actionCooldownFeedMs = ms; }
        void setCleanCooldown(unsigned long ms){ actionCooldownCleanMs = ms; }
        void setTrainCooldown(unsigned long ms){ actionCooldownTrainMs = ms; }
};